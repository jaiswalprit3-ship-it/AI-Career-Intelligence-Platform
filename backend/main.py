...
async def create_resume(
    ...
    if resume_text and resume_text.strip():
        ...
        extracted_text = extract_text_from_file(content, file_type)
...
# =========================
# LIST RESUMES (CLIENT BASED)
# =========================
@app.get("/resumes", response_model=List[ResumeResponse])
async def get_resumes(client_id: str = Query(...)):
    result = (
...
async def analyze_skill_gap(resume_id: str):
    ...
    text = resume.data[0]["resume_text"]
    analysis = gemini_client.generate_text(f"Analyze skill gaps:\n{text}")
...
async def analyze_career(resume_id: str):
    ...
    text = resume.data[0]["resume_text"]
    recommendations = gemini_client.generate_text(f"Career advice:\n{text}")
...
async def analyze_roadmap(resume_id: str):
    ...
    text = resume.data[0]["resume_text"]
    roadmap = gemini_client.generate_text(f"Learning roadmap:\n{text}")

    roadmap_id = str(uuid.uuid4())
    created_at = datetime.utcnow().isoformat()

    supabase.table("learning_roadmaps").insert(
        {
            "id": roadmap_id,
            "resume_id": resume_id,
            "roadmap": roadmap,
